...
def create_decision(client, person_id, decision="rejected"):
    response = client.post(
        f"/api/v1/persons/{person_id}/action-plan/decisions",
        json={"decision": decision, "note": "用户决策"},
    )
    assert response.status_code == 201
    return response.json()
...
def seed_confirmed_decision(person_id):
    with get_connection() as conn:
        return ActionDecisionRepository.create(
            ...
            person_id,
            "recommendation-seeded-for-feedback-test",
            "confirmed",
            "测试确认",
        )
...
def test_action_feedback_synthesizes_rejected_decision_without_outcome(client):
    person = create_person(client)
    create_relationship(client, person["id"])
...
def test_action_feedback_isolated_by_user(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    response = client.get(
