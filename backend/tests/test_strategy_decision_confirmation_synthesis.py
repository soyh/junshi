...
def seed_decision(person_id, recommendation_id, decision="confirmed"):
    with get_connection() as conn:
        return ActionDecisionRepository.create(
            conn, USER_ID, person_id, recommendation_id, decision, "合成测试"
        )
...
def test_confirmation_synthesis_is_person_isolated(client):
    ...
    decision = seed_decision(first["id"], "recommendation-a", "confirmed")
...
def test_confirmation_synthesis_is_deterministic(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    decision = seed_decision(person["id"], "recommendation-a", "confirmed")
