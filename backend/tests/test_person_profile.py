...
def test_person_profile_aggregates_existing_person_data(client):
    ...
    relationship = client.post(
        ...
        json={"person_id": person_id, "status": "active", "stage": "dating", "long_term_goal": "建立长期关系", "current_goal": "增加互动", "notes": "profile test"},
    )
    assert relationship.status_code == 201
    conversation = client.post(
        "/api/v1/conversations",
        json={"person_id": person_id, "title": "测试会话", "status": "active"},
    )
    assert conversation.status_code == 201
    conversation_id = conversation.json()["id"]
    message = client.post(
        "/api/v1/messages",
        json={"conversation_id": conversation_id, "sender_type": "person", "content": "今天有空吗？", "sent_at": "2026-08-23T10:00:00+00:00"},
    )
    assert message.status_code == 201
    interaction = client.post(
        "/api/v1/interactions",
        json={"person_id": person_id, "relationship_id": relationship.json()["id"], "type": "meeting", "occurred_at": "2026-08-23T11:00:00+00:00", "content": "一起喝咖啡"},
    )
...
def test_person_profile_isolated_by_user(client):
    person = create_person(client)
    other_user_headers = {"X-User-ID": "11111111-1111-1111-1111-111111111111"}
    response = client.get(f"/api/v1/persons/{person['id']}/profile", headers=other_user_headers)
...
def test_person_profile_does_not_mix_other_person_interactions(client):
    ...
    interaction = client.post(
        "/api/v1/interactions",
        json={"person_id": second["id"], "type": "message", "occurred_at": "2026-08-23T12:00:00+00:00", "content": "属于对象B"},
    )
    assert interaction.status_code == 201
