...
def test_outcome_feedback_learning_reaches_fresh_analysis_and_recommendation(client):
    ...
    with get_connection() as conn:
        decision = ActionDecisionRepository.create(
            conn, USER_ID, person["id"], "recommendation-previous", "confirmed", "用户显式确认"
        )
    ...
    class RecordingProvider:
        ...
        def analyze(self, context):
            ...
            return {
                ...
                "observed_facts": [{"content": "上一轮行动已产生结果", "confidence": 1.0, "evidence_source_ids": [message["id"]]}],
                "inferences": [],
                "unknowns": [{"content": "长期影响未知", "confidence": 1.0, "evidence_source_ids": [message["id"]]}],
                "hypotheses": [{"content": "下一轮保持低压力沟通", "confidence": 0.7, "evidence_source_ids": [message["id"]]}],
                "emotional_signals": [],
    ...
    service = AnalysisRecommendationService(
        analysis_llm_service=AnalysisLLMService(provider=provider),
    )
    with get_connection() as conn:
        result = service.build_context(conn, USER_ID, conversation["id"], provider=provider)

    assert len(provider.contexts) == 1
    learning = provider.contexts[0]["learning_strategy"]["learning_inputs"]["action_feedback"]
    ...
    assert learning[0]["unknown_outcome_count"] == 0
