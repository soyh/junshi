from app.core.database import get_connection
from app.repositories.action_decision import ActionDecisionRepository


USER_ID = "00000000-0000-0000-0000-000000000001"


def create_person(client, name="推荐学习策略对象"):
    response = client.post("/api/v1/persons", json={"name": name})
    assert response.status_code == 201
    return response.json()


def create_relationship(client, person_id):
    response = client.post(
        "/api/v1/relationships",
        json={"person_id": person_id, "status": "active", "stage": "dating"},
    )
    assert response.status_code == 201


def seed_decision(person_id, recommendation_id="recommendation-a"):
    with get_connection() as conn:
        return ActionDecisionRepository.create(
            conn,
            USER_ID,
            person_id,
            recommendation_id,
            "confirmed",
            "推荐学习策略桥接测试决策",
        )


def create_outcome(client, person_id, decision_id, outcome="completed"):
    execution = client.post(
        f"/api/v1/persons/{person_id}/strategy-decision/executions/{decision_id}",
        json={"note": "测试执行"},
    )
    assert execution.status_code == 201

    response = client.post(
        f"/api/v1/persons/{person_id}/action-plan/outcomes/{decision_id}",
        json={"outcome": outcome, "note": "推荐学习策略桥接测试结果"},
    )
    assert response.status_code == 201
    return response.json()


def get_context(client, person_id):
    return client.get(f"/api/v1/persons/{person_id}/recommendation-analysis/context")


def test_recommendation_context_exposes_learning_strategy_candidates(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    decision = seed_decision(person["id"], "recommendation-observed")
    create_outcome(client, person["id"], decision["id"], "completed")

    response = get_context(client, person["id"])
    assert response.status_code == 200
    learning = response.json()["learning_strategy"]

    assert learning["candidates"][0]["recommendation_id"] == "recommendation-observed"
    assert learning["candidates"][0]["observed_outcome_count"] == 1
    assert learning["strategy_decision_learning"]["learning_candidate_decision_ids"] == [decision["id"]]
    assert learning["strategy_decision_learning"]["recommendation_observed_counts"] == {
        "recommendation-observed": 1
    }


def test_recommendation_context_preserves_learning_unknowns(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    decision = seed_decision(person["id"], "recommendation-unknown")
    create_outcome(client, person["id"], decision["id"])

    learning = get_context(client, person["id"]).json()["learning_strategy"]
    bridge = learning["strategy_decision_learning"]

    assert bridge["unknown_decision_ids"] == []
    assert learning["candidates"][0]["synthesis_status"] == "source_backed_candidate"
    assert "recommendation_quality" in learning["candidates"][0]["unknowns"]
    assert learning["constraints"]["must_preserve_learning_unknowns"] is True
    assert learning["constraints"]["must_not_turn_learning_into_fact"] is True


def test_recommendation_context_keeps_unobserved_decisions_unknown(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    decision = seed_decision(person["id"], "recommendation-pending")

    learning = get_context(client, person["id"]).json()["learning_strategy"]
    bridge = learning["strategy_decision_learning"]

    assert learning["candidates"] == []
    assert bridge["learning_candidate_decision_ids"] == []
    assert bridge["unknown_decision_ids"] == [decision["id"]]
    assert bridge["unknown_count"] == 1


def test_recommendation_context_learning_strategy_is_person_isolated(client):
    first = create_person(client, "推荐学习A")
    second = create_person(client, "推荐学习B")
    create_relationship(client, first["id"])
    create_relationship(client, second["id"])
    decision = seed_decision(first["id"], "recommendation-a")
    create_outcome(client, first["id"], decision["id"])

    learning = get_context(client, second["id"]).json()["learning_strategy"]
    assert learning["candidates"] == []
    assert learning["strategy_decision_learning"]["learning_candidate_decision_ids"] == []
    assert learning["strategy_decision_learning"]["unknown_decision_ids"] == []


def test_recommendation_context_learning_strategy_is_user_isolated(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    response = client.get(
        f"/api/v1/persons/{person['id']}/recommendation-analysis/context",
        headers={"X-User-ID": "11111111-1111-1111-1111-111111111111"},
    )
    assert response.status_code == 404


def test_recommendation_context_learning_strategy_is_deterministic_and_read_only(client):
    person = create_person(client)
    create_relationship(client, person["id"])
    decision = seed_decision(person["id"], "recommendation-a")
    create_outcome(client, person["id"], decision["id"])

    first = get_context(client, person["id"])
    second = get_context(client, person["id"])
    assert first.status_code == 200
    assert second.status_code == 200
    assert second.json() == first.json()
    assert second.json()["learning_strategy"]["constraints"]["must_not_auto_execute"] is True
    assert second.json()["learning_strategy"]["constraints"]["must_not_auto_send"] is True
