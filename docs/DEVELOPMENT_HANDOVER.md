...
更新时间：2026-09-17
当前阶段：TEST-116 — Account Credentials / Login → Server-issued Session — VERIFIED
当前 Branch：test-116-account-login-session
服务器验收代码 HEAD：`bf84a5c068813693715fa0b09ce5458d59b7356e`
...
TEST-114 VERIFIED — Production Authentication Boundary；服务器 full 578 passed；服务器验收代码 HEAD `693946882ca780eafc0367dfa26a3b7f0ea06f84`。
...
3. 静态合法 token 映射到 `LOCAL_USER_ID`，作为 bootstrap compatibility；
4. 缺 token / 错 token → 401；production 未配置 auth token 且无 active session → 503 fail closed；
5. development/test 暂保留 legacy `X-User-ID` 兼容；
6. auth token 纳入日志脱敏。
...
5. `POST /api/v1/auth/sessions` 只能为已认证的自己创建 session；
6. `DELETE /api/v1/auth/session` 只吊销当前真实 DB session；
7. 不同 session 可以解析到不同 users，并继续复用既有 scope isolation。

服务器验收：session 7、TEST-114 auth regression 7、scope isolation 4、full 585 passed in 95.17s；工作树 clean；migration diff 仅 `011_auth_sessions.sql`。

## TEST-116 — Account Credentials / Login → Server-issued Session — VERIFIED

目标：普通用户无需静态 bootstrap token 即可注册/登录，并由服务器签发 TEST-115 opaque session；客户端不能决定 user_id。

已建立：
1. migration 012 `user_credentials`：`user_id` 主键/外键、唯一 normalized `username`、`password_hash`、timestamps；
2. `POST /api/v1/auth/register`：服务器生成 UUID `users.id`，写入 credential 并签发 session；
3. `POST /api/v1/auth/login`：username/password 验证成功后签发新的 session；
4. username trim + lowercase normalization，长度 3~64；
5. credential request 使用 `SecretStr` 且 `extra=forbid`，拒绝客户端传 `user_id`；
6. 密码使用 `hashlib.scrypt` + 随机 salt，参数 `N=16384, r=8, p=1`，数据库不保存明文密码；
7. wrong password 与 unknown username 使用相同 `401 invalid credentials`；未知账号仍执行 dummy hash verification；
8. 登录产生的新 session 与注册 session 映射同一 server-side user scope；
9. 不建立第二套 token/session 实现；不修改 migration 001~011。
...
- full pytest：592 passed in 102.24s；
- `git status --short` blank；
- migration diff：仅 `A backend/migrations/012_user_credentials.sql`；
- 最终文件差异与 GitHub 预期一致。
...
TEST-117 优先：
1. 多设备 session list / current-session identification；
2. user-scoped revoke-one / revoke-other sessions；
3. 当前 session rotation，旧 token 立即失效；
4. 为静态 `AUTH_BEARER_TOKEN` 增加显式 disable/retirement 开关，在不破坏历史 VERIFIED contract 的情况下建立退场路径；
5. 后续再做登录暴力尝试防护、password change/recovery、release/runtime security 和完整 login UI。
